...
public partial class Special_Offer : System.Web.UI.Page
{
    ...
    public void BindData()
    {
        ...
        dt = Obj_Book.get_all_specialoffer_book();

        string a1 = "<div class='panel'>" +
                        "<div class='titnewre'>" + newrelease.ToString() + " </div>" +
                        "<a href='" + Config.WebSiteMain + "Category.aspx?l=" + System.Threading.Thread.CurrentThread.CurrentCulture.Name + "' class='titnewre1'>" + viewallbook.ToString() + "</a>" +
                    "</div>";
        string str = "";
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                str = str + "<div class='namkl2'><a href = '" + bookDetailUrl + "' style='color:#616161'>" + dr["Title1"] + "</a></div>" +
                      "<div class='author'>" + dr["Autoher"] + "</div>";
                if (Convert.ToBoolean(dr["IsFree"]) == true)
                {
                    str = str + "<div class='namkl' style='width: 20%;'>" + isfreetitle.ToString() + "</div>";
                    str = str + "<div class=\"namkl\" style=\"width: 80%;float:right;text-align: right;color: red;\">" + dr["RemainingTime"] + "</div>";
                }
                else
                {
                    str = str + "<div class='namkl' style='width: 20%;'>$" + dr["FinalPrice"] + "</div>";
                    str = str + "<div class=\"namkl\" style=\"width: 80%;float:right;text-align: right;color: red;\">" + dr["RemainingTime"] + "</div>";
                }
                if (Convert.ToBoolean(dr["IsFree"]) == true)
                {
                    str = str + "<a href='" + Config.WebSiteMain + "Index.aspx?fbid=" + S.Encrypt(dr["BookID"].ToString()) + "&l=" + System.Threading.Thread.CurrentThread.CurrentCulture.Name + "' class='boxbut'>" + addtolibrarytitle.ToString() + "</a>" +
                    "</div>";
                }
                else
                {
                    str = str + "<a href='" + Config.WebSiteMain + "Index.aspx?id=" + S.Encrypt(dr["BookID"].ToString()) + "&l=" + System.Threading.Thread.CurrentThread.CurrentCulture.Name + "&catid=" + Request.QueryString["catid"] + "' class='boxbut'>" + buynowtitle.ToString() + "</a>" +
                    "</div>";
                }
            }
            str = str + "</div>";
        ...
        if (dt.Rows.Count == 0)
        {
            str += "<div style=\"font-family: 'abeezeeregular';font-size: 15px;font-weight: normal;margin-top: 50px;\">" + ResourceManager.GetString("noSpecialOffer", System.Threading.Thread.CurrentThread.CurrentCulture.Name) + "</div>";
    ...
    [WebMethod]
    public static string BindDatas()
    {
        BookBAL Obj_Book = new BookBAL();
        Security S = new Security();
        string isfreetitle = "";
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                if (Convert.ToBoolean(dr["IsFree"]) == true)
                {
                    str = str + "<a href='" + Config.WebSiteMain + "Index.aspx?fbid=" + HttpUtility.UrlEncode(S.Encrypt(Convert.ToString(dr["BookID"]))) + "&l=" + culture + "' class='boxbut'>" + addtolibrarytitle.ToString() + "</a>" +
                "</div>";
                }
                else
                {
                    str = str + "<a href='" + Config.WebSiteMain + "Index.aspx?id=" + HttpUtility.UrlEncode(S.Encrypt(Convert.ToString(dr["BookID"]))) + "&l=" + culture + "' class='boxbut'>" + buynowtitle.ToString() + "</a>" +
                    "</div>";
                }
        ...
        if (dt.Rows.Count == 0)
        {
            string NoData = "<div id=\"divNoData\" onload=\"hideAlert()\"></div>";
            return NoData;
        }
        return str;
    }
