...
public partial class PaymentNew : Page
{
    ...
    bool IspaperBookFound = false;

    protected void Page_Load(object sender, EventArgs e)
    {

        //txt_Url.Text = Session["Url"].ToString();
    ...
    protected void BindData()
    {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    if (dt.Rows[i]["IsPaperBook"].ToString() == "True")
                    {
                        ...
                        IspaperBookFound = true;
                        Delivery.Style.Add("display", "Block");
                        Addredd.Style.Add("display", "Block");
                    }
                }
        ...
        if (dt != null && dt.Rows.Count > 0)
        {
            //for(int i=0; i>dt.Rows.Count; i++)
            //{
            //    if(Convert.ToString(dt.Rows[i]["Quantity"])=="0")
            //    {
        ...
        if (IspaperBookFound == false)
        {
            lblShipAmount.Visible = false;
            LocalizedLiteral17.Visible = false;
            LocalizedLiteral18.Visible = false;
            // LocalizedLiteral24.Visible = false;
            chkdhl.Visible = false;
    ...
    protected void btn_submit_Click(object sender, EventArgs e)
    {
        ...
        lbl_loader.Visible = true;
        var id = objBookAddDetail.InsertBookAddress();
        Response.Redirect(Request.RawUrl);
        btn_submit.Visible = false;
        //objBookAddDetail.Name = "";
        //objBookAddDetail.StreetAddress = "";
        //objBookAddDetail.Landmark = "";
        //objBookAddDetail.City = "";
        //objBookAddDetail.State = "";
        //objBookAddDetail.Country = "";
        //objBookAddDetail.Pincode = "";
        //objBookAddDetail.PhoneNumber = "";
        //txtname.Text = "";
        //txtaddress.Text = "";
        //txtlandmark.Text = "";
        //txtcity.Text = "";
    ...
    public string getRemoveString12()
    {
        //OnClientClick="return confirm('" + '<%# ResourceManager.GetString("Are you sure you want to Remove?",System.Threading.Thread.CurrentThread.CurrentCulture.Name) %>' + "'"); ";
        return "return confirm('" + Localization.ResourceManager.GetString("Are you sure you want to Remove?", System.Threading.Thread.CurrentThread.CurrentCulture.Name) + "');";
        //return "";
    ...
    protected void chkDeliver_CheckedChanged(object sender, EventArgs e)
    {
        ...
        //string query = "UPDATE BookDeliveryAddressDetail SET IsDefault=1  WHERE BookDeliveryAddID = " + hfAddID.Value;
        //DAL.SqlHelper.ExecuteNonQuery(CommandType.Text, query);

        BookDeliveryAddressBAL objBookAddress = new BookDeliveryAddressBAL();
        string customerID = (S.Decrypt(HttpUtility.UrlDecode(Request.QueryString["UserID"])).ToString());
        string address = hfAddID.Value;
        ...
        string query = "UPDATE BookDeliveryAddressDetail SET IsDefault = 0 WHERE UserID =" + customerID;
        DAL.SqlHelper.ExecuteNonQuery(CommandType.Text, query);
        string query1 = "UPDATE BookDeliveryAddressDetail SET IsDefault= 1  WHERE BookDeliveryAddID = " + hfAddID.Value.Trim() + " and UserID=" + customerID;
        DAL.SqlHelper.ExecuteNonQuery(CommandType.Text, query1);
    ...
    protected void btn_login_Click(object sender, EventArgs e)
    {
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            {
                if (Convert.ToString(dt.Rows[k]["Quantity"]) == "0" && Convert.ToBoolean(dt.Rows[k]["IsPaperBook"].ToString()))
                {
                    //Response.Write("<script>alert('This Book Was Deleted...')</script>");
                    Session["Quantity"] = dt.Rows[k]["Quantity"];
                    ScriptManager.RegisterStartupScript(this, GetType(), "for", "alert('Please Remove Out of Stock Book...');", true);
                    //Response.Redirect(Request.RawUrl);
                    break;

                }
                if (Convert.ToString(dt.Rows[k]["Quantity"]) != "0" || Convert.ToBoolean(dt.Rows[k]["IseBook"].ToString()))
                {
                    var checkPaper = 0;
                            ...
                            for (int i = 0; i < rptRecords1.Items.Count; i++)
                            {
                                ...
                                if (chk != null && chk.Checked)
                                {
                                    if (hfAmount != null && hfAdd != null)
                                    {
                                        ...
                                        if (chkdhl.Checked == true)
                                        {
                                            Response.Redirect("Payment.aspx?l=" + Thread.CurrentThread.CurrentCulture.Name + "&UserID=" + Convert.ToInt32(S.Decrypt(HttpUtility.UrlDecode(Request.QueryString["id"]))) + "&ShippingCharge=" + lbldhl.Text + "&Address=" + address + "&ShippingType=" + ShippingType + "&totalAmount="+ lblAmount.Text);

                                        }
                                        else
                                        {
                                            Response.Redirect("Payment.aspx?l=" + Thread.CurrentThread.CurrentCulture.Name + "&UserID=" + Convert.ToInt32(S.Decrypt(HttpUtility.UrlDecode(Request.QueryString["id"]))) + "&ShippingCharge=" + lblShipAmount1.Text + "&Address=" + address + "&ShippingType=" + ShippingType + "&totalAmount=" + lblAmount.Text);
                                        }
    ...
    {
        //if (ddlregion.SelectedItem.Text.ToUpper() == "SELECT REGION")
        //{
        //    ddlcityi.Items.Insert(0,"Select City");
        //    ddlCountry.Items.Insert(0,"Select Country");
        //}
    ...
    protected void ddlregrpt_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        tb2.Items.Insert(0, "Select City");
    }
    ...
    {
        var name = System.Threading.Thread.CurrentThread.CurrentCulture.Name.ToString();
        name = name == "" || name == null ? "es-ES" : name;
        return name;
    }
